...
func (r *LibraryLockerRule) Meta() rules.Rule {
	return r.Rule
}

func (r *LibraryLockerRule) Check(node *reader.RichNode, context map[string]interface{}, filepath string) *rules.Finding {
	...
	for i := 2; i < len(node.Children); i++ {
		...
		if child.Type == reader.NodeString {
			// Functions with docstrings are documented public API contracts, not useless wrappers
			return nil
		}
