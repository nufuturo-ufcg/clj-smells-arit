...
func (r *InappropriateCollectionRule) Meta() rules.Rule {
	return rules.Rule{
...
func (r *InappropriateCollectionRule) Check(node *reader.RichNode, _ map[string]interface{}, filepath string) *rules.Finding {
	if node.Type != reader.NodeList || len(node.Children) < 2 {
		return nil
	}
...
func init() {
	rules.RegisterRule(&InappropriateCollectionRule{})
}