...
func (r *CyclicDependencyRule) Meta() rules.Rule {
	return r.Rule
}

func (r *CyclicDependencyRule) Check(node *reader.RichNode, context map[string]interface{}, filepath string) *rules.Finding {
	cyclicDepMutex.Lock()
...
func findMutualCycle(graph map[string]map[string]*reader.RichNode, visitedFuncs map[string]bool, filepath string) *rules.Finding {
...
func init() {
	...
	rules.RegisterRule(NewCyclicDependencyRule())
