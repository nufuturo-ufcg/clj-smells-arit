...
func (r *DuplicatedCodeRule) Meta() rules.Rule {
	return r.Rule
}

func (r *DuplicatedCodeRule) Check(node *reader.RichNode, context map[string]interface{}, filepath string) *rules.Finding {
	initRegex()
...
func (r *DuplicatedCodeRule) checkForDuplication(block CodeBlockInfo, filepath string) *rules.Finding {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func init() {
	rules.RegisterRule(&DuplicatedCodeRule{
		Rule: rules.Rule{
			ID:          "duplicated-code",
